...
export function formatMessageContent(content: MessageContent): string {
  ...
  if (Array.isArray(content)) {
    return content.map((block: ContentBlock): string => {
      ...
      if (block.type === 'tool_use') {
        ...
        const jsonStr = JSON.stringify(toolBlock.input || {}, null, 2).replace(/```/g, '\\`\\`\\`');
      ...
      if (block.type === 'tool_result') {
        ...
        const rawResult = (resultBlock as unknown as Record<string, unknown>).run as { result?: unknown } | undefined;
        ...
        if (nestedResult !== undefined) {
          ...
          resultContent = resultBlock.content.map((c: string | ToolResultContentItem): string => {
